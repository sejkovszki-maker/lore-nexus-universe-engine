...
test('source library exposes audited sources works and claims without mobile overflow',async({page})=>{await page.setViewportSize({width:390,height:844});await page.goto('/#/sources');await expect(page.getByRole('heading',{name:'Forrástár és műjegyzék'})).toBeVisible();await expect(page.getByRole('button',{name:/Források \(29\)/})).toBeVisible();await page.getByRole('button',{name:/Művek \(32\)/}).click();await expect(page.getByRole('heading',{name:'Diablo III: Book of Cain'})).toBeVisible();await expect(page.getByText('gyűjtemény: Tales of Sanctuary').first()).toBeVisible();await page.getByRole('button',{name:/Ellenőrzött állítások \(12\)/}).click();await expect(page.getByText('Diablo II: Lord of Destruction után és Diablo III előtt')).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth-window.innerWidth)).toBeLessThanOrEqual(1);});
