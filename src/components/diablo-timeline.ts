...
export class DiabloTimeline extends LitElement {
  ...
  private openRelated(articleId:string){const article=wikiArticles[articleId];if(article?.type==='book'){const book=storyBooks(this.activeUniverseId).find(item=>item.id===articleId);useAppStore.openBookRoute(articleId,book?.chapters[0]?.id);return;}useAppStore.openArticleRoute(articleId);}
