...
export class BookLibrary extends LitElement {
  ...
  render(){
    ...
    if(!book) return html`<section aria-labelledby="books-title"><h1 id="books-title">Könyvek</h1><p>A könyvek külön is olvashatók. A történetolvasóban a megadott kronológiai ponton jelennek meg.</p><div class="grid">${books.map(item=>html`<article class="card"><h2>${item.title}</h2><p>${item.chapters.length?`${item.chapters.length} fejezet`:'Ismertető és bibliográfia'}${item.after?` • a történetben „${wikiArticles[item.after]?.title || item.after}” után`:''}</p><button @click=${()=>useAppStore.openBookRoute(item.id,item.chapters[0]?.id)}>${item.chapters.length?'Könyv olvasása':'Könyvadatlap megnyitása'}</button></article>`)}</div>${books.length?'':html`<p>Ehhez az univerzumhoz még nincs külön olvasható könyv.</p>`}</section>`;
